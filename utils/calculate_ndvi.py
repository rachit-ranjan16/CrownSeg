...
from datapaths import DATASET_PATH, HYPER_BANDS_FILE, HYPER_SOURCE_PATH, CONFIG_FILE, NDVI_DEST_PATH
...
class GDALCalcNDVI():
    ...
    def createOutputImage(self, outFilename, inDataset):
        ...
        if gdal.DCAP_CREATE in metadata and metadata[gdal.DCAP_CREATE] == 'YES':
            log.info('Driver GTiff supports Create() method.')
        else:
    ...
    def calcNDVI(self, in_file, out_file):
        ...
        red_band = dataset.GetRasterBand(int(config['NDVI']['RED_BAND'])) # RED BAND
...
if __name__ == '__main__':
    ...
    for hyper_file in hyper_files[0:1]:
        log.debug('Tackling HyperFile=%r', hyper_file)
        ndvi.calcNDVI(HYPER_SOURCE_PATH + hyper_file, NDVI_DEST_PATH + hyper_file)
